...
const AddressSearchBar = ({ title, intl, handleAddressChange }) => {
  ...
  const locationInfoText = currentLocation
    ? intl.formatMessage({ id: 'address.search.location' }, { location: currentLocation })
    : '';
  // Figure out which info text to use
  let infoText = showSuggestions && addressResults.length ? (
    <FormattedMessage id="search.suggestions.suggestions" values={{ count: addressResults.length }} />
  ) : (
    locationInfoText
  );
  if (cleared) {
        ...
        <StyledFlexContainer>
          <StyledInputBase
            ...
            inputProps={{
              ...
              'aria-label': `${intl.formatMessage({ id: 'search.searchField' })} ${intl.formatMessage({
                id: 'address.search',
              })}`,
              'aria-owns': showSuggestions ? 'address-results' : null,
              'aria-activedescendant':
                showSuggestions && resultIndex !== null ? `address-suggestion${resultIndex}` : null,
            }}
            type="text"
          ...
          <Typography aria-live="polite" id="resultLength" style={visuallyHidden}>
            {infoText}
          </Typography>
        </StyledFlexContainer>
        {showSuggestions ? (
          <Paper>
            <List role="listbox" id="address-results">
              {addressResults.map((address, i) => (
                ...
                >
                  <Typography>{getAddressText(address, getLocaleText)}</Typography>
                </ListItem>
              ))}
            </List>
          </Paper>
